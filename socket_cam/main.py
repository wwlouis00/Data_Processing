...
from public_method import create_image_windows, initialize
                ...
                try:
                    ...
                    try:
                        ...
                        if not len(k):
                            ...
                            try:
                                CA_msg = Calibration.calibrate(framerate, iso)
                            ...
                            """
                            baseline_begin = int(int(k[7:12])/5) # 00035 -> 7, 7 means the seventh data of detection.csv, which is 3.5 minutes
                            baseline_end = int(int(k[13:18])/5) # 00320 -> 64, 64 means the sixty-forth data of detection.csv, which is 32 minutes
                        ...
                        sleep(1)
                        print("No message from server, but server is still connected.")
                        continue
